...
const apiUrl = `http://localhost:3000/task/user/day?userId=${userId}&day=${diaDaSemana}`;
...
function getDiaDaSemana() {
    ...
    return diasDaSemana[diaIndex]; // Retorna o nome do dia correspondente
}
...
async function loadTasks() {
    try {
        ...
        tasks.forEach((task) => {
            ...
            time.textContent = `${task.startHour} - ${task.endHour}`; // Preenche o horário
